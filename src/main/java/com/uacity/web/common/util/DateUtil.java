package com.uacity.web.common.util;
...
public class DateUtil {
	...
	public static int getCurYear(Date date) {
		return getYearAndMonthAndDay(date)[0];
	}
	...
	public static int getCurMonth(Date date) {
		return getYearAndMonthAndDay(date)[1];
	}
	...
	public static int getCurDay(Date date) {
		return getYearAndMonthAndDay(date)[2];
	}
	...
	public static void main(String[] args) throws ParseException {
		...
		Date date = DateUtil.DateStringToDate(DateUtil.dateToString(new Date(),
				amask));
		List list = null;
		if (getCurDay(date) <= dayi) {
			list = getDateLaterList(amask, getFirstDayOfMonthStr(date), dayi
					- getFirstDayOfMonth(date) + 1);
		} else {
			list = getDateLaterList(
					amask,
					getAssignDateNext(amask, getFirstDayOfMonthStr(date), dayi),
					getLastDayOfMonth(date) - dayi);
		}
